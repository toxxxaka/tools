...
# Печатает эффективный sshd -T, опционально для пользователя, Match/Include из файлов, SFTP и слушатели FTP
check_sshd_config() {
    hr 'Конфигурация sshd'
    ...
    if [ "$(id -u)" -ne 0 ]; then
        printf '\033[33mНе root: sshd -T может быть неполным.\033[0m\n'
        printf 'Для полной картины запустите скрипт с sudo.\n\n'
    fi
    ...
    # Частые директивы аутентификации, сети, сессий и переносов
    _sshd_pat='^(port|listenaddress|addressfamily|usepam|permitrootlogin|pubkeyauthentication|passwordauthentication|kbdinteractiveauthentication|challengeresponseauthentication|maxauthtries|logingracetime|maxstartups|maxsessions|clientaliveinterval|clientalivecountmax|allowtcpforwarding|x11forwarding|tcpkeepalive|usedns|allowusers|denyusers|allowgroups|denygroups|authorizedkeysfile|banner|gatewayports|compression|permitemptypasswords|printmotd|printlastlog|permittty)'
    printf '\033[1mЭффективные параметры (sshd -T), глобально:\033[0m\n\n'
    printf '%s\n' "$out" | grep -E "$_sshd_pat" \
        || printf '(нет совпадений по основному фильтру)\n'
    printf '\n\033[1mSFTP / принудительное окружение (часть из sshd -T):\033[0m\n\n'
    printf '%s\n' "$out" | grep -E '^(subsystem|chrootdirectory|forcecommand|authorizedkeyscommand|authorizedprincipalsfile) ' \
        || printf '(нет subsystem / chroot / forcecommand в эффективном конфиге)\n'

    printf '\nИмя пользователя для \033[1msshd -T -C user=...\033[0m (Enter — пропустить): '
    read -r ssh_ctx_user
    ssh_ctx_user=$(printf '%s' "$ssh_ctx_user" | tr -d '[:space:]')
    if [ -n "$ssh_ctx_user" ]; then
        if out_u=$(sshd -T -C "user=$ssh_ctx_user" 2>/dev/null); then
            printf '\n\033[1mЭффективные параметры для user=%s:\033[0m\n\n' "$ssh_ctx_user"
            printf '%s\n' "$out_u" | grep -E "$_sshd_pat"
            printf '\n'
            printf '%s\n' "$out_u" | grep -E '^(subsystem|chrootdirectory|forcecommand) ' \
                || true
        else
            printf '\033[31msshd -T -C user=%s не выполнился.\033[0m\n' "$ssh_ctx_user"
        fi
    fi

    printf '\n\033[1mMatch / Include в файлах (не развёрнуто в -T):\033[0m\n\n'
    if [ -f /etc/ssh/sshd_config ]; then
        grep -nE '^[[:space:]]*(Match|Include)[[:space:]]' /etc/ssh/sshd_config 2>/dev/null \
            || printf '  (нет Match/Include в /etc/ssh/sshd_config)\n'
    fi
    for f in /etc/ssh/sshd_config.d/*.conf; do
        [ -f "$f" ] || continue
        hits=$(grep -nE '^[[:space:]]*(Match|Include)[[:space:]]' "$f" 2>/dev/null)
        if [ -n "$hits" ]; then
            printf '--- %s ---\n' "$f"
            printf '%s\n' "$hits"
        fi
    ...
    printf '\n\033[1mFTP не относится к sshd\033[0m — слушатели порта 21 (если есть):\n\n'
    if is_installed ss; then
        ss -tulpn 2>/dev/null | grep -E ':21[[:space:]]' \
            || printf '  (ничего не слушает :21 или ss без прав)\n'
    else
        netstat -tulpn 2>/dev/null | grep -E ':21[[:space:]]' \
            || printf '  (нет данных по :21)\n'
    fi
    for name in vsftpd proftpd pure-ftpd; do
        if is_installed "$name"; then
            printf '  установлен: %s\n' "$name"
        fi
    done

    printf '\n\033[1mПодсказка:\033[0m для chroot SFTP смотрите subsystem sftp, internal-sftp, ChrootDirectory в Match; для FTP — отдельные демоны и firewall.\n'
    hr
}

# В проде можно включить: удаление скрипта после выхода по 0
#harakiri() {
...
printf '
    ...
    4. Firewall
    5. Fail2ban
    6. Listen ports + Web servers (80 / 443, Caddy)
    7. Nginx
    8. Apache
   ...
   16. Конфигурация sshd
   17. Docker / Podman (ps -a)
    0. Exit
...
case "$action" in
        ...
        if is_installed caddy; then
            ...
            if is_installed systemctl; then
                ...
                systemctl status caddy.service --no-pager -l 2>/dev/null | head -n 15 \
                    || printf '  (unit не найден или недоступен)\n'
            fi
        else
            printf 'caddy не установлен\n'
        fi
    ...
    10)
        ...
        # TCP к домену (SYN + установление), не только ICMP
        ...
        # HTTP: только успех/неуспех на уровне приложения, без дампа заголовков
